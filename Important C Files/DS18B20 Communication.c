...
// the sequence requires a total 
// of about 80 microseconds to read one bit.
unsigned char readDQbit(void){
unsigned char bitValue = 1;
    ...
    // and monitor to see when DQ goes high.
    // If it remains low for more then 60 microseconds,
    // then incoming bit is a 0.
    // Bus goes high immediately if bit is a 1.
    if (WaitForTristate() > 60) bitValue = 0;  
    
    // Allow total of 80 microseconds to elapse
    while (!mT4GetIntFlag());    
    return(bitValue);
}
